...
int CEntity::NetworkClipped(int SnappingClient, vec2 CheckPos)
{
	...
	float dx = GameServer()->GetPlayer(SnappingClient)->m_ViewPos.x - CheckPos.x;
	float dy = GameServer()->GetPlayer(SnappingClient)->m_ViewPos.y - CheckPos.y;

	if (absolute(dx) > 1000.0f || absolute(dy) > 800.0f)
		return 1;

	if (distance(GameServer()->GetPlayer(SnappingClient)->m_ViewPos, CheckPos) > 1100.0f)
		return 1;
	return 0;
}
